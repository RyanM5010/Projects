...
export const selectUserCheckoutAddress = (address) => {
    localStorage.setItem("CHECKTOUT_ADDRESS", JSON.stringify(address))
    return {
...
export const createStripePaymentSecret  = 
    (totalPrice) => async (dispatch, getState)  =>{
        try {
            ...
            console.log(error);
            toast.error(error?.response?.data?.message || "Fail to create client secret");
        } 
};
...
export const stripePaymentConfirmation 
    = (sendData, setErrorMessage, setLoading, toast) => async (dispatch, getState) => {
        try {
            const response = await api.post("/order/users/payment/online", sendData);
            if (response.data){
