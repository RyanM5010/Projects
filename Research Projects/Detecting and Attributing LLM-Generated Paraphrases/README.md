...
1. **Sentence Pair Paraphrase Source Detection (SP-PSD)**  
   Classify which sentence in a pair is LLM-generated given the original.

2. **Sentence Pair Paraphrase Authorship Attribution (SP-PAA)**  
   Determine which sentence (or both/neither) in a paraphrase pair was generated by an LLM.

3. **Single Sentence Authorship Attribution (SS-AA)**  
   Identify whether a single sentence was written by a human or generated by an LLM.

4. **Original vs. Paraphrased LLM Classification (Para-LLM-Disc)**  
   Distinguish between an LLM’s original response and its paraphrased version.
