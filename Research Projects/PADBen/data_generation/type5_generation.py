...
class EnhancedGeminiParaphraser:
    ...
    async def iterative_paraphrase(self, 
                                  ...
                                  target_length: int, 
                                  max_length: int, 
                                  prompt_template: str) -> IterationHistory:
...
class Type5Generator:
    ...
    def get_method_column_name(self, method: Type4ParaphraseMethod, iteration_level: IterationLevel) -> str:
        """Get the appropriate column name for the paraphrasing method and iteration level."""
        method_name_map = {
    ...
    async def paraphrase_with_dipper(self, context: Type5Context) -> Type5Result:
        """Perform iterative paraphrasing using DIPPER model."""
        try:
            ...
            if not final_text:
                return Type5Result(
                    ...
                    metadata={"failure_reason": "No iterations completed", "method": "dipper"}
                )
            ...
            logger.error(f"DIPPER iterative paraphrasing failed: {str(e)}")
            return Type5Result(
                final_paraphrased_text=None,
                method_used=Type4ParaphraseMethod.DIPPER,
                iterations_completed=0,
                ...
                metadata={"failure_reason": str(e), "method": "dipper"}
            )
    
    async def paraphrase_with_prompt(self, context: Type5Context) -> Type5Result:
        """Perform iterative paraphrasing using Gemini prompt-based method."""
        try:
            ...
            history = await self.gemini_paraphraser.iterative_paraphrase(
                ...
                context.max_length,
                self.config.paraphrase_prompt_template
            )
            ...
            if not final_text:
                return Type5Result(
                    ...
                    success=False,
                    iteration_history=history,
                    metadata={"failure_reason": "No iterations completed", "method": "prompt_based"}
                )
            ...
            logger.error(f"Prompt-based iterative paraphrasing failed: {str(e)}")
            return Type5Result(
                final_paraphrased_text=None,
                method_used=Type4ParaphraseMethod.PROMPT_BASED,
                iterations_completed=0,
                ...
                success=False,
                iteration_history=IterationHistory(context.generated_text, [], [], []),
                metadata={"failure_reason": str(e), "method": "prompt_based"}
            )
    ...
    async def paraphrase_text(self, 
        ...
        try:
            ...
            generated_text = row.get('llm_generated_text', '')
    ...
    def filter_target_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Filter data for Type 5 generation."""
        ...
        # Filter to samples missing LLM-paraphrased generated text
        mask &= (df['llm_paraphrased_generated_text'].isna() | (df['llm_paraphrased_generated_text'] == ''))
        
        # Ensure we have required Type 2 generated text
        mask &= df['llm_generated_text'].notna() & (df['llm_generated_text'] != '')
    ...
    async def generate_for_dataset(self, 
        ...
        """Generate Type 5 paraphrases for the entire dataset."""
        self.stats["start_time"] = datetime.now()
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        # Get appropriate directory structure
        final_output_dir = self.get_directory_structure(output_dir, iteration or self.config.default_iteration, timestamp)
        ...
        method_column = self.get_method_column_name(method or self.config.default_method, iteration_level)
        ...
        for batch_idx in range(total_batches):
            ...
            for idx, result in batch_results:
                if result.success and result.final_paraphrased_text:
                    results_df.at[idx, method_column] = result.final_paraphrased_text
        ...
        # Save final results
        await self._save_final_results(results_df, paraphrase_metadata, method_name, iteration_level.name, final_output_dir)
    ...
    async def _save_intermediate_results(self, df: pd.DataFrame, metadata: List[Dict], batch_num: int, output_dir: str):
        ...
        try:
            ...
            checkpoint_file = output_path / f"type5_checkpoint_batch_{batch_num}.csv"
            df.to_csv(checkpoint_file, index=False)
            
            # Save metadata checkpoint
            metadata_file = output_path / f"type5_metadata_batch_{batch_num}.json"
            with open(metadata_file, 'w', encoding='utf-8') as f:
                json.dump(metadata, f, indent=2, default=str, ensure_ascii=False)
            
            logger.info(f"Saved Type 5 checkpoint after batch {batch_num}")
        except Exception as e:
            logger.warning(f"Failed to save intermediate results: {str(e)}")
    
    async def _save_final_results(self, df: pd.DataFrame, metadata: List[Dict], method_name: str, iteration_name: str, output_dir: str):
        """Save final Type 5 paraphrasing results."""
        try:
            output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    async def run_type5_generation():
        ...
        method_column = generator.get_method_column_name(method, iterations)
