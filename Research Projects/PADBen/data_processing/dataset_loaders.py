...
class OUTFOXLoader:
    """Specialized loader for OUTFOX dataset."""
    ...
    def load_and_validate(self) -> pd.DataFrame:
        """Load and validate OUTFOX dataset."""
        logger.info("Loading OUTFOX dataset...")
        
        # TODO: Implement actual OUTFOX loading when path is provided
        if self.data_path and self.data_path.exists():
            logger.info(f"Loading OUTFOX from {self.data_path}")
            # Implementation depends on actual OUTFOX format
            pass
        else:
            logger.warning("OUTFOX data path not provided or doesn't exist")
            # Return empty DataFrame with proper structure
            self.df = pd.DataFrame({
                'human_text': [],
                'generated_text': []
    ...
    def get_text_types(self) -> Dict[int, pd.Series]:
        """Get available text types from OUTFOX."""
        if self.df is None:
            raise ValueError("Dataset not loaded. Call load_and_validate() first.")
        
        if self.df.empty:
            return {i: pd.Series([], dtype='object') for i in [1, 2]}
        
        return {
            1: self.df['human_text'],     # Human original
            2: self.df['generated_text'] # LLM generated
        } 